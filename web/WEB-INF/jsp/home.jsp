...
<link href="styles/home.css" rel="stylesheet" type="text/css" media="screen" />
<style type="text/css">
...
$(document).ready(function(){
	...
	function getAllDep(){
		...
		$.ajax({
			...
			success:function(json){
				 ...
				 getUserByDep(jsonData.data[0].depId);
			},error:function(){
				jAlert("网络数据异常！","提示");
			},complete:function(){
				//组别背景切换
	...
	function getUserByDep(depId){
		$.ajax({
			url:"json_UserAction_getUserByDep.action",
			data:"departmentModel.depId="+depId,
			type:"POST",
			dataType:"json",
			cache:false,
	...
	function delUserById(userId,depId){
		$.ajax({
			url:"json_UserAction_delUserById.action",
			data:"userModel.isDel=1&userModel.userId="+userId,
			type:"POST",
			dataType:"json",
			cache:false,
			error:function(){
	...
	$("#addDepOKBtn").click(function(){
		...
		$.ajax({
			...
			data:"departmentModel.depName="+depName+"&departmentModel.depDesc="+depDesc,
			cache:false,
			success:function(){
...
<tr>
	...
	<td lang=${'${'}userId} tag={{= userName}}><input class="edituser" title="编辑" type="image" src="images/usermgr/pencil.png">
...
<li>
<font>${'${'}depName}</font>
<div class="grpbtn" lang={{= depId}}>
	<input type="image" src="images/usermgr/addgrp.png">
	...
	<div style="text-align: center;width: 550px;height: 220px;margin: 0 auto;">
		<table style="width: 550px;height: 210px">
			...
			<tr>
				<td>用户类型:</td>
				<td>
					<select name="userType" style="width: 180px; height: 30px;">
